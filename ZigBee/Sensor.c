...
int fn_NetSim_Zigbee_SensorEvent(int nSensorLoop,NETSIM_ID nGlobalPANCoordinatorId,AGENT** pstruAgent,METRICS** pstruMetrics,NetSim_EVENTDETAILS* pstruEventDetails)
{
	...
	for(nAgentLoop =0;nAgentLoop<MAXAGENT;nAgentLoop++)
	{
		...
		//noise_at_event_time = fn_netsim_matlab_run((NETWORK->pstruSimulationParameter->dVal)/1000, (pstruEventDetails->dEventTime)/1000);
		
		dDistance = fn_Sensor_CalculateDistance(pstruTemppos,pstruPos);
		if(dDistance <= pstru_Sensor[nSensorLoop]->nSensorRange)
		{
...
#ifdef _SENSOR_INFORMATION
			
			noise_at_event_time = get_sensor_info(PstruPacket);
#else
			sprintf(str,"%lf",noise_at_event_time);
