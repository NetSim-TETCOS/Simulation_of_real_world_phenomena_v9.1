	...
	case PHYSICAL_OUT_EVENT:
		{
			...
			for(nLoop=1; nLoop<=NETWORK->ppstruNetSimLinks[nLink_Id-1]->puniDevList.pstruMP2MP.nConnectedDeviceCount; nLoop++)
			{
				...
				WSN_PHY(ncon)->dTotalReceivedPower += dCummulativeReceivedPower[pstruPacket->nTransmitterId-1][ncon-1];
				...
				if(ncon==pstruPacket->nReceiverId || pstruPacket->nReceiverId == 0)
				{
					...
					if(WSN_PHY(ncon)->dTotalReceivedPower - dCummulativeReceivedPower[pstruPacket->nTransmitterId-1][ncon-1] >= WSN_PHY(ncon)->dReceiverSensivity)
						packet->nPacketStatus = PacketStatus_Collided;

					if(MW_DBM(dCummulativeReceivedPower[pstruPacket->nTransmitterId-1][ncon-1]) > MW_DBM(WSN_PHY(ncon)->dReceiverSensivity)*2)
					{
			...
			case UPDATE_MEDIUM:
				{
					...
					for(nLoop=1; nLoop<=NETWORK->ppstruNetSimLinks[nLink_Id-1]->puniDevList.pstruMP2MP.nConnectedDeviceCount; nLoop++)
					{
						...
						{
							WSN_PHY(ncon)->dTotalReceivedPower -= dCummulativeReceivedPower[nTransmitterID-1][ncon-1];
...
{
	fprintf(stderr, "Initializing MATLAB\n");
	fn_matlab_init();
...
{
	fn_netsim_matlab_run((NETWORK->pstruSimulationParameter->dVal)/1000000,0,0,2);
	fprintf(stderr,"Press any key to close MATLAB\n");
	getch();
...
{
	#ifdef _SINKNODE_INFORMATION
		get_sinknode_info(pstruEventDetails->pPacket);
